...
namespace LCU.State.API.Forge.Infrastructure.Harness
{
    ...
    public class IDGraph : IdentityGraph
    {
        ...
        #region API Methods
        public override async Task<string> RetrieveThirdPartyAccessToken(string entApiKey, string email, string key)
        {
            return await withG(async (client, g) =>
            {
        ...
        public override async Task<Status> SetThirdPartyAccessToken(string entApiKey, string email, string key, string token)
        {
            return await withG(async (client, g) =>
            {
                var registry = email.Split('@')[1];
